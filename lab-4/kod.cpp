...
std::vector<bool> asciiToBinary(const std::string& text) {
...
void ASK(std::vector<bool>& binaryResult, std::vector<double>& N_values, std::vector<double>& out, double A1, double A2, double fn, double M, double Tb, double fs, double B) {
    double t;
    for (int n = 0; n < B; n++) {
...
void PSK(std::vector<bool>& binaryResult, std::vector<double>& N_values, std::vector<double>& out, double fn, double M, double Tb, double fs, double B) {
    double t;
    for (int n = 0; n < B; n++) {
        if (binaryResult[n] == 0) {
            for (int m = 0; m < M; m++) {
...
void FSK(std::vector<bool>& binaryResult, std::vector<double>& N_values, std::vector<double>& out, double fn1, double fn2, double M, double Tb, double fs, double B) {
    double t;
    for (int n = 0; n < B; n++) {
        if (binaryResult[n] == 0) {
            for (int m = 0; m < M; m++) {
...
int main() {
    ...
    std::string input = "mielony";
    std::vector<bool> binaryResult = asciiToBinary(input);
    ...
    PSK(binaryResult, N_values2, out2, fn, M, Tb, fs, B);
    FSK(binaryResult, N_values3, out3, fn1, fn2, M, Tb, fs, B);
    plt::figure();
    plt::plot(N_values3, out3, "b-");
    plt::xlabel("czas");
    plt::ylabel("z(a)");
    plt::title("sygnal zf");
    plt::show();
    ...
    for (int k = 0; k < N; ++k) {
        tab_re.push_back(cx_out[k].r);
        tab_im.push_back(cx_out[k].i);
    }
    ...
    //DFT(N, out, tab_re, tab_im, fk_values, M_value, fs);
    ...
    std::cout << "left : " << fk_values[left_index] << std::endl;
    std::cout << "right : " << fk_values[right_index] << std::endl;
    double width = fk_values[right_index] - fk_values[left_index];
